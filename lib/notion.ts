...
type UrlProperty = Extract<PageObjectResponse['properties'][string], { type: 'url' }>
...
export function getFilesFromProperty(property: FilesProperty): string[] {
    return property.files.map(file => isInternalFileProperty(file) ? file.file.url : isExternalFileProperty(file) ? file.external.url : '');
}
